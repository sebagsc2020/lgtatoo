// ============================================================
// 🤖 OPTIMIZACIÓN SEO AUTOMÁTICA (Blindada) - LG Tattoo
// ============================================================
...
class SEOOptimizer {
    ...
    generateMetaTags() {
        try {
            ...
            console.warn('⚠️ Error en generateMetaTags:', e.message);
        }
    }

    getPageTitle() {
        const hash = window.location.hash ? window.location.hash.replace('#', '') : 'inicio';
        const titles = {
        ...
        return titles[hash] || titles['inicio'];
    }
    ...
    getPageDescription() {
        ...
        return descriptions[hash] || descriptions['inicio'];
    }
    ...
    getPageKeywords() {
        ...
        return pageKeywords[hash] || baseKeywords;
    }
    ...
    generateJSONLD() {
        try {
            ...
            if (document.querySelector('script[data-seo-jsonld]')) return;
    ...
    generateOpenGraph() {
        try {
            ...
            console.warn('⚠️ Error en generateOpenGraph:', e.message);
        }
    }
    ...
    analyzeContent() {
        try {
            const mainContent = document.querySelector('main') ? document.querySelector('main').textContent : '';
    ...
    setupCanonicalUrls() {
        try {
            ...
            canonical.href = this.siteUrl + (window.location.pathname || '') + (window.location.hash || '');
        } catch (error) {
    ...
    addBreadcrumbs() {
        try {
            // Eliminar breadcrumb anterior si existe para evitar acumulación
            const existingBC = document.querySelector('script[data-seo-breadcrumb]');
            if (existingBC) existingBC.remove();

            const breadcrumb = document.createElement('script');
            breadcrumb.type = 'application/ld+json';
            breadcrumb.setAttribute('data-seo-breadcrumb', 'true');
